...
//    LOG(ERROR) << "EMPTY KEYFRAME TIMES!";
    ...
    std::fstream pgTimeSaveStream(save_directory + "times.txt", std::fstream::out);
...
void DataSaver::saveOdometryVerticesTUM(std::vector<nav_msgs::Odometry> keyframePosesOdom) {
    ...
    for (int i = 0; i < keyframePosesOdom.size(); i++) {
        ...
        stream << time << " " << odometry.pose.pose.position.x << " " << odometry.pose.pose.position.y
               << " " << odometry.pose.pose.position.z << " "
               << odometry.pose.pose.orientation.x << " " << odometry.pose.pose.orientation.y << " "
               << odometry.pose.pose.orientation.z << " " << odometry.pose.pose.orientation.w << std::endl;
    }
}
...
void DataSaver::saveLoopandImagePair(std::map<int, int> loopIndexCheckedMap,
...
//  LOG(INFO) << "WRITE Lidar LOOP FILE: " << save_directory + "lidar_loop.txt";

    int j = 0;
    for (auto it = loopIndexCheckedMap.begin(); it != loopIndexCheckedMap.end(); ++it) {
...
//
//    if (saveLoopdata) {
//      std::string common_name = std::to_string(curr_node_idx) + "_" + std::to_string(prev_node_idx);
//
//      std::string pcd_name_0 = common_name + "_0.pcd";
//      std::string pcd_name_1 = common_name + "_1.pcd";
//      pcl::io::savePCDFileBinary(pgScansDirectory + pcd_name_0, *keyframeLaserRawClouds[curr_node_idx]);
//      pcl::io::savePCDFileBinary(pgScansDirectory + pcd_name_1, *keyframeLaserRawClouds[prev_node_idx]);
//
////      cv::imwrite(pgImageDirectory + common_name + "_0_0.png", keyMeasures.at(curr_node_idx).camera0.front());
////      cv::imwrite(pgImageDirectory + common_name + "_0_1.png", keyMeasures.at(curr_node_idx).camera1.front());
////      cv::imwrite(pgImageDirectory + common_name + "_0_2.png", keyMeasures.at(curr_node_idx).camera2.front());
////      cv::imwrite(pgImageDirectory + common_name + "_0_3.png", keyMeasures.at(curr_node_idx).camera3.front());
////      cv::imwrite(pgImageDirectory + common_name + "_0_4.png", keyMeasures.at(curr_node_idx).camera4.front());
////      // cv::imshow("imgCallback", image_mat);
////
////      cv::imwrite(pgImageDirectory + common_name + "_1_0.png", keyMeasures.at(prev_node_idx).camera0.front());
////      cv::imwrite(pgImageDirectory + common_name + "_1_1.png", keyMeasures.at(prev_node_idx).camera1.front());
////      cv::imwrite(pgImageDirectory + common_name + "_1_2.png", keyMeasures.at(prev_node_idx).camera2.front());
////      cv::imwrite(pgImageDirectory + common_name + "_1_3.png", keyMeasures.at(prev_node_idx).camera3.front());
////      cv::imwrite(pgImageDirectory + common_name + "_1_4.png", keyMeasures.at(prev_node_idx).camera4.front());
//      cv::imwrite(pgImageDirectory + common_name + "_1_4.png", resultMat_vec.at(j));
//    }
        j++;
        loop_outfile.precision(15);
        loop_outfile << std::to_string(curr_node_idx) << " " << std::to_string(prev_node_idx);
...
//  LOG(INFO) << "SAVE LOOP FILE: " << loopIndexCheckedMap.size();
    ...
    camera_pair_outfile.open(save_directory + "camera_pair_indices.txt", std::ios::out);
//  LOG(INFO) << "WRITE CAMERA PAIR FILE: " << save_directory + "camera_pair_indices.txt";
//  LOG(INFO) << "Matching camera size: " << all_camera_corre_match_pair.size();
    for (const auto &camera_pair: all_camera_corre_match_pair) {
        ...
        camera_pair_outfile << lidar_idx_1 << " " << lidar_idx_2 << " " << cam_idx_1 << " " << cam_idx_2 << " "
                            << num_corr
                            << std::endl;
    }
...
void DataSaver::savePointCloudMap(std::vector<nav_msgs::Odometry> allOdometryVec,
...
void DataSaver::savePointCloudMap(pcl::PointCloud<PointT> cloud_ptr) {
    ...
    pcl::io::savePCDFileBinaryCompressed(save_directory + "globalmap_binaryCompressed.pcd", cloud_ptr);
}
