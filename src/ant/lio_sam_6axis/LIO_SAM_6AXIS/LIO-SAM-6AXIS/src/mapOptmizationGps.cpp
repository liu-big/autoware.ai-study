...
//    std::queue<nav_msgs::Odometry::ConstPtr> odometryBuf;
    ...
    mapOptimization() {
        ...
        subCloud = nh.subscribe<lio_sam_6axis::cloud_info>("lio_sam_6axis/feature/cloud_info",
                                                           1,
                                                           &mapOptimization::laserCloudInfoHandler,
                                                           this,
                                                           ros::TransportHints().tcpNoDelay());

        subGPS = nh.subscribe<nav_msgs::Odometry>(gpsTopic,
                                                  200,
                                                  &mapOptimization::gpsHandler,
                                                  this,
                                                  ros::TransportHints().tcpNoDelay());
        ...
        pubHistoryKeyFrames =
                nh.advertise<sensor_msgs::PointCloud2>("lio_sam_6axis/mapping/icp_loop_closure_history_cloud", 1);
        pubIcpKeyFrames =
...
#pragma omp parallel for num_threads(numberOfCores)
    ...
    bool saveMapService(std_srvs::Empty::Request &req, std_srvs::Empty::Response &res) {
        ...
        if(saveDebugFile){
            ...
            dataSaverPtr->saveResultBag(keyframePosesOdom, allResVec);  //保存odom数据包
            nh.param<bool>("lio_sam_6axis/useGPS", useGPS, false);
            if (useGPS)
...
//    }
    ...
    void
    loopFindNearKeyframes(pcl::PointCloud<PointType>::Ptr &nearKeyframes, const int &key, const int &searchNum) {
