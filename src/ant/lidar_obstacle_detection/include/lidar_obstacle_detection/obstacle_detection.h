...
#include "lidar_obstacle_detection/DetectionConfig.h"
...
namespace ObstacleDetection_ns
{
    ...
    private:
        ...
        ros::Subscriber cloud_sub,current_velocity_sub_;
        ros::Publisher raw_cloud_pub, cloud_pub, box_pub, box_pub_ab,box_car_pub, speed_ratio_pub;
        ros::Publisher box_stop_pub,box_range_pub,box_car2_pub;
        ...
        bool safe_detection(Box box_point,double speed_ratio[]);
        double car_length, car_width, car_hight;
        double lidar_front_dis;
        double front_stop_distance, edge_stop_distance, back_stop_distance;
        //double speed_ratio[3];
        double cur_velocity;

        jsk_recognition_msgs::BoundingBox box_car,box_car2,box_stop,box_range;

        ros::NodeHandle node;

        dynamic_reconfigure::Server<lidar_obstacle_detection::DetectionConfig> server;
        void reconfigureCB(lidar_obstacle_detection::DetectionConfig &config, uint32_t level);


        void callbackCurrentVelocity(const geometry_msgs::TwistStampedConstPtr &msg);
        //jsk_recognition_msgs::BoundingBox box_car;
        double filterRes, cluster_distance, x_range, y_range, z_range;
        int min_points_num, max_points_num;
        double x_stop_range_front,x_stop_range_rear, y_stop_range, z_stop_range;
    };
}
