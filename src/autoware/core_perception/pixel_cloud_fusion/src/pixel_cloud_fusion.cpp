...
void ROSPixelCloudFusionApp::CloudCallback(const sensor_msgs::PointCloud2::ConstPtr &in_cloud_msg)
{
  ...
  pcl::PointCloud<pcl::PointXYZRGB>::Ptr out_cloud1(new pcl::PointCloud<pcl::PointXYZRGB>);
  std::vector<int> numb_(in_cloud->points.size());
  ...
  {
    cam_cloud[i] = TransformPoint(in_cloud->points[i], camera_lidar_tf_);  // 投影
    ...
    out_cloud1->points.push_back(colored_3d_point);
  }
#if 0
  out_cloud->points.clear();
  out_cloud1->points.clear();
  pcl::copyPointCloud(*in_cloud,*out_cloud1);   // pcl::PointXYZ转为pcl::PointXYZRGB
  int cout=0;
#pragma omp for
  for (int row = 0; row < image_size_.height; row++)
  {
    for (int col = 0; col < image_size_.width; col++)
    {
      ...
      if (iterator_3d_2d != projection_map.end())
      {
        ...
        out_cloud1->points[numb_[cout]] = colored_3d_point;
        cout++;
  ...
  pcl::toROSMsg(*out_cloud1, cloud_msg);
