...
static pcl::PointCloud<pcl::PointXYZ>::Ptr my_map(new pcl::PointCloud<pcl::PointXYZ>);
...
// Default values
static int max_iter = 50;       // Maximum iterations
...
static std::string _map_path = "/home/lao/work/autoware.ai/point_map/";
static float _filter_res = 0.2;
static int _save_points = 1800000;
static int _intercept_points = 450000;
...
//     std::cerr << "1111111111111111111111111 " << std::endl;
...
//     std::cerr << "33333333333333333333333333333 " << std::endl;
//   }
...
//   if (filter_res == 0.0)
//   {
//     // map_filtered->erase(map_filtered->begin(),map_filtered->begin()+100000);//截取前面多出的点云

//     pcl::io::savePCDFileASCII(filename, *map_ptr);
//     std::cout << "Saved " << map_ptr->points.size() << " data points to " << filename << "." << std::endl;
//         std::cerr << "444444444444444444444444444 " << std::endl;
//   // }
//   // else
//   // {
//     // pcl::io::savePCDFileASCII(filename, *map_filtered);
//     // std::cout << "Saved " << map_filtered->points.size() << " data points to " << filename << "." << std::endl;
//     //     std::cerr << "55555555555555555555555555555555555555 " << std::endl;

//       pcl::PointCloud<pcl::PointXYZ>::Ptr map_copy(new pcl::PointCloud<pcl::PointXYZ>);

//       pcl::copyPointCloud(*map_ptr, *map_copy);//src中的xyz覆盖掉src_PN中的xyz值，然后把xyz+normal的信息给src_P
//       map_copy->erase(map_copy->begin(),map_copy->end()-map.points.size()/2);//删除第一个
//       map.points.clear();			// 释放掉智能指针托管的指针内存，并将其置NULL
//       pcl::copyPointCloud(*map_copy, map);//src中的xyz覆盖掉src_PN中的xyz值，然后把xyz+normal的信息给src_PN
//       // pcl::io::savePCDFileASCII("/home/lao/work/autoware.ai/point_map/test.pcd", map);
//   }
// }
...
void points_callback(const sensor_msgs::PointCloud2::ConstPtr input)
{
    ...
    if (flag) //获取第一帧数据
    {
        ...
        filename.append(_map_path).append(std::to_string(0)).append("_map.pcd");
        // pcl::PointCloud<pcl::PointXYZI>::Ptr map_filtered(new pcl::PointCloud<pcl::PointXYZI>(map));

        pcl::copyPointCloud(map, *my_map); // src中的xyz覆盖掉src_PN中的xyz值，然后把xyz+normal的信息给src_P
        // pcl::io::savePCDFileASCII(filename, *my_map);
    }
    ...
    printf("-map.points.size()- %d\n", map_ptr->points.size());
    std::cerr << map_ptr->points.size() << " , " << map.points.size() << std::endl;
    std::cerr << map_ptr->width << " , " << map_ptr->height << std::endl;

    if (map_ptr->points.size() == map_ptr->width * map_ptr->height){
        
        // static pcl::PointCloud<pcl::PointXYZI> map_pub_data;
        // map_pub_data += *map_ptr;
        pcl::toROSMsg(*map_ptr, *map_msg_ptr);
        ndt_map_pub.publish(*map_msg_ptr);
    }
    std::cerr << "HHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHH " << std::endl;
    q.setRPY(current_pose.roll, current_pose.pitch, current_pose.yaw);
...
// void points_callback_2(sensor_msgs::PointCloud2 input)
// {
...
static void output_callback(const autoware_config_msgs::ConfigNDTMappingOutput::ConstPtr &input)
{
    ...
    pcl::PointCloud<pcl::PointXYZI>::Ptr map_ptr(new pcl::PointCloud<pcl::PointXYZI>(map));
    pcl::PointCloud<pcl::PointXYZI>::Ptr map_filtered(new pcl::PointCloud<pcl::PointXYZI>());
    if (input->filter_res == 0)
    {

        pcl::io::savePCDFileASCII(input->filename, *map_ptr);
        std::cout << "output map_ptr!!!!!" << std::endl;
    ...
    else
    {
        ...
        voxel_grid_filter.setInputCloud(map_ptr);
        voxel_grid_filter.filter(*map_filtered);
        pcl::io::savePCDFileASCII(input->filename, *map_filtered);
        std::cout << "Original: " << map_ptr->points.size() << " points." << std::endl;
        std::cout << "Filtered: " << map_filtered->points.size() << " points." << std::endl;
        std::cout << input->filename << std::endl;
    }
...
void My_Thread()
{
    ...
    while (ros::ok())
    {
        static long int points_size = 0;
        if (points_size != map.points.size())
        {
            points_size = map.points.size();
            flag_num = 0;
        }
        flag_num++;
        // std::cout << "flag_num: " << flag_num << std::endl;
        if (flag_num > 600000)
        {
            flag_num = 0;
            if (map.points.size() > 5000)
            {

                pcl::PointCloud<pcl::PointXYZI>::Ptr map_ptr(new pcl::PointCloud<pcl::PointXYZI>(map));
                pcl::PointCloud<pcl::PointXYZI>::Ptr map_filtered(new pcl::PointCloud<pcl::PointXYZI>());

                pcl::PointCloud<pcl::PointXYZI>::Ptr frist_map(new pcl::PointCloud<pcl::PointXYZI>());
                pcl::copyPointCloud(*my_map, *frist_map); //把第一帧数据赋给map
                *map_ptr = *frist_map + *map_ptr;

                // map_ptr->header.frame_id = "map";
                // map_filtered->header.frame_id = "map";

                if (num > 1)
                    map_ptr->erase(map_ptr->begin(), map_ptr->begin() + 450000); //先删除前一次点云再保存

                if (_filter_res == 0)
                {
                    std::cout << "output map_ptr ! ! ! ! !" << std::endl;
                    std::cout << "Original: " << map_ptr->points.size() << " points." << std::endl;
                    pcl::io::savePCDFileASCII(_map_path + "back_map.pcd", *map_ptr);
                }
                else
                {
                    pcl::VoxelGrid<pcl::PointXYZI> voxel_grid_filter;
                    voxel_grid_filter.setLeafSize(_filter_res, _filter_res, _filter_res);
                    voxel_grid_filter.setInputCloud(map_ptr);
                    voxel_grid_filter.filter(*map_filtered);
                    ...
                    pcl::io::savePCDFileASCII(_map_path + "back_map.pcd", *map_filtered);
                }
            }
        }

        if ((long int)map.points.size() > _save_points)
        {
            // ROS_ERROR("-map.points.size()- %d, %d", map.points.size()，map_ptr->points.size())
            num++;

            // double filter_res = 0.2;
            std::string filename;
            // str = sprintf(str,"%ld",num);
            filename.append(_map_path).append(std::to_string(num)).append("_map.pcd");
            std::cout << "output_callback" << std::endl;
            std::cout << "filter_res: " << _filter_res << std::endl;
            std::cout << "filename: " << filename << std::endl;
            ...
            if (num > 1)
                map_ptr->erase(map_ptr->begin(), map_ptr->begin() + _intercept_points); //先删除前一次点云再保存

            //   // pcl::copyPointCloud(*my_map, *map_ptr);//把数据赋给map
            pcl::PointCloud<pcl::PointXYZI>::Ptr frist_map(new pcl::PointCloud<pcl::PointXYZI>());
            pcl::copyPointCloud(*my_map, *frist_map); //把第一帧数据赋给map
            *map_ptr = *frist_map + *map_ptr;
            ...
            map.erase(map.begin(), map.end() - _intercept_points); //保存后一截点云
            ...
            // pcl::io::savePCDFileASCII("/home/lao/work/autoware.ai/point_map/test.pcd", *map_copy);
            std::cout << "Saved " << map_filtered->points.size() << " data points to " << filename << "." << std::endl;

            // pcl::PointCloud<pcl::PointXYZI>::Ptr map_copy(new pcl::PointCloud<pcl::PointXYZI>(map));
            // // pcl::copyPointCloud(*map_filtered, *map_copy);//src中的xyz覆盖掉src_PN中的xyz值，然后把xyz+normal的信息给src_P
            // map_copy->erase(map_copy->begin(),map_copy->end()-450000);//删除第一个
            // map.points.clear();			// 释放掉智能指针托管的指针内存，并将其置NULL
            // // pcl::copyPointCloud(*my_map, map);//把第一帧数据赋给map
            // pcl::copyPointCloud(*map_copy, map);//把数据赋给map
            // points_num = map.points.size();
        }
    }
}
...
int main(int argc, char **argv)
{
    ...
    private_nh.getParam("map_path", _map_path);
    private_nh.getParam("filter_res", _filter_res);
    private_nh.getParam("save_points", _save_points);
    private_nh.getParam("intercept_points", _intercept_points);
    ...
    std::cout << "map_path: " << _map_path << std::endl;
    std::cout << "filter_res: " << _filter_res << std::endl;
    std::cout << "save_points: " << _save_points << std::endl;
    std::cout << "intercept_points: " << _intercept_points << std::endl;
