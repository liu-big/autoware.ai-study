...
#include <time.h>
#define TOSTR(_FILENAME_, _NUM_, _SUFFIX_) #_FILENAME_ #_NUM_ "." #_SUFFIX_
...
pcl::PointXYZRGB single_rgb_;
// static pcl::PointCloud<pcl::PointXYZRGB> map_rgb_point_;
//  bool map_clear_flag=false;
...
void output_callback(std::string file_, double filter_res_)
{
    double filter_res = filter_res_;
    ...
    map_ptr->points.erase(map_ptr->points.begin(), map_ptr->points.end() - map_ptr->points.size() / 2);
    map.points.erase(map.points.begin(), map.points.end() - map.points.size() / 2);
...
static void points_callback(const sensor_msgs::PointCloud2::ConstPtr &input)
{
    ...
    for (pcl::PointCloud<pcl::PointXYZRGB>::const_iterator item = tmp.begin(); item != tmp.end(); item++)
    {
        ...
        p.b = (double)item->b;
        // 获取gpu颜色
        single_rgb_.r = (double)item->r;
        single_rgb_.g = (double)item->g;
        single_rgb_.b = (double)item->b;
    ...
    pcl::PointCloud<pcl::PointXYZRGB>::Ptr map_ptr(new pcl::PointCloud<pcl::PointXYZRGB>(map)); //每次都会重新赋值,每次都比map少一帧
    ...
    {
        map += *transformed_scan_ptr; // 添加了该帧数据
        ...
        if (count_ % 20 == 0)
        {
            autoware_config_msgs::ConfigNDTMappingOutput conf_;
            conf_.filter_res = 0.1;
            int num_ = int(count_ / 20);
            // std::string filename = TOSTR(/home/huazai/work/AutowareAuto/autoware/install/startingup_ros/share/startingup_ros/lidar_match, num_,pcd);
            std::string filename_;
            filename_ = pcd_dir + "/lidar_match_" + std::to_string(num_) + ".pcd";
            conf_.filename = filename_;
            // output_callback((const autoware_config_msgs::ConfigNDTMappingOutput::ConstPtr)&conf_);
            output_callback(filename_, 0.1);

            initial_scan_loaded = 0;
            // is_first_map = true;
            map.points.erase(map.points.begin(), map.points.end() - map.points.size() / 2);
            // map.points.clear();
            //  memcpy(&map, &map_rgb_point_, map.points.size() / 2);
        }
        count_++;
        // for (int i; i < transformed_scan_ptr->points.size();i++)
        // {
        //     pcl::PointCloud<pcl::PointXYZRGB> rgb_point_;
        //     rgb_point_.x =
        // }
        // int err = map_ptr->size() - map_rgb_point_.size();
        // if(err < 0)
        // {
        //     map_rgb_point_.points.clear();
        // }else{
        //     memcpy(map_ptr[], , err)
        // }
    }
...
int main(int argc, char **argv)
{
    ...
    private_nh.getParam("pcd_dir", pcd_dir);
