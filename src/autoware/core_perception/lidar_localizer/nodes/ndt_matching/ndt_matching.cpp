...
static void gnss_callback(const geometry_msgs::PoseStamped::ConstPtr& input)
{
  ...
  double diff_xt,diff_yt,diff_zt,diff_tt=0.0;
  if(_use_gnss == 1){
    diff_xt = current_pose.x - current_gnss_pose.x;
    diff_yt = current_pose.y - current_gnss_pose.y;
    diff_zt = current_pose.z - current_gnss_pose.z;
    diff_tt = sqrt(diff_xt * diff_xt + diff_yt * diff_yt + diff_zt * diff_zt);

    ROS_WARN("fitness_score: %f ",fitness_score);
  }
  if ((_use_gnss == 1 && init_pos_set == 0) || (_use_gnss == 1 && fitness_score >= _gnss_reinit_fitness) || (diff_tt > 30.0 ))
  {
