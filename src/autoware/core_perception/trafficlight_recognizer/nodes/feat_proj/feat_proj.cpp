...
void echoSignals2_new(const ros::Publisher& pub, bool useOpenGLCoord = false)
{
  int countPoint = 0;
  autoware_msgs::Signals signalsInFrame;
  ...
  for (const auto& signal_map : vmap.signals)
  {
    ...
    int u, v;
    if (true)//project2(signalcenter, &u, &v, useOpenGLCoord) == true)
    {
      countPoint++;
      ...
      // signal_angle will be zero if signal faces to x-axis
      // Target signal should be face to -50 <= z-axis (= 90 degree) <= +50
      if (true)//isRange(-50, 50, signal_angle - 90))
      {
        signalsInFrame.Signals.push_back(sign);
      }
    }
...
int main(int argc, char* argv[])
{
  ...
  while (true)
  {
    ...
    #else
      echoSignals2_new(signalPublisher, false);
    #endif
    loop.sleep();
  }
}
