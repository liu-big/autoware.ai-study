...
//
// static int gLowH = 40;
// static int gHighH = 70;
// static int gLowS = 90;
// static int gHighS = 255;
// static int gLowV = 1;
// static int gHighV = 255;
...
int main(int argc, char* argv[])
{
  ...
  // cv::createTrackbar("LowH","Threshold",&gLowH,179);
  // cv::createTrackbar("HighH","Threshold",&gHighH,179);

  // cv::createTrackbar("LowS","Threshold",&gLowS,255);
  // cv::createTrackbar("HighS","Threshold",&gHighS,255);

  // cv::createTrackbar("LowV","Threshold",&gLowV,255);
  // cv::createTrackbar("HighV","Threshold",&gHighV,255);
  ...
  thSet.Green.Sat.lower = 0.2f;   //DAYTIME_S_SIGNAL_THRESHOLD;
  thSet.Green.Val.upper = 1.0f;
  thSet.Green.Val.lower = (80.0/255.0) ;//DAYTIME_V_SIGNAL_THRESHOLD;

  // thSet.Green.Hue.upper = static_cast<double>(gLowH);
  // thSet.Green.Hue.lower = static_cast<double>(gHighH);
  // thSet.Green.Sat.upper = static_cast<double>(gHighH);
  // thSet.Green.Sat.lower = static_cast<double>(gLowS);
  // thSet.Green.Val.upper = static_cast<double>(gHighV);
  // thSet.Green.Val.lower = static_cast<double>(gLowV);
  ...
  // 传输交通灯的颜色状态话题 camera_light_color_topic_name
