                              ...
                              pcl::PointCloud<pcl::PointXYZ>::Ptr out_cloud_ptr)
{
  ...
  for (unsigned int i = 0; i < in_cloud_ptr->points.size(); i++)
  {
    ...
    // if (in_cloud_ptr->points[i].x < _lidar_back_range) 
    //    continue;
...
void velodyne_callback(const sensor_msgs::PointCloud2ConstPtr &in_sensor_cloud)
{
    ...
    if (!_using_sensor_cloud)
    {
        ...
        {
            removePointsUpTo(filtered_cloud_ptr, removed_points_cloud_ptr, _remove_points_upto);
            //removePointsUpTo(current_sensor_cloud_ptr, removed_points_cloud_ptr, _remove_points_upto);
        }
        else
        {    
            removed_points_cloud_ptr = filtered_cloud_ptr;
