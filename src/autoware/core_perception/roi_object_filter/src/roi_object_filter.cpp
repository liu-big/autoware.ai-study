...
void RosRoiObjectFilterApp::publishCloudClusters(const autoware_msgs::DetectedObjectArray &in_detections)
{
    autoware_msgs::CloudClusterArray clusters_objects;
    clusters_objects.header = in_detections.header;
    clusters_objects.header.stamp = ros::Time::now();
    clusters_objects.header.frame_id = "map";

    for (size_t i = 0; i < in_detections.objects.size(); i++)
    {
        autoware_msgs::CloudCluster cluster_object;
        cluster_object.header = in_detections.header;
        cluster_object.label = "";
        cluster_object.score =  0.0;

        cluster_object.header.frame_id = "velodyne" ;//in_detections.objects[i].space_frame;
        cluster_object.bounding_box.pose = in_detections.objects[i].pose;
        cluster_object.dimensions = in_detections.objects[i].dimensions;
        cluster_object.cloud = in_detections.objects[i].pointcloud;
        cluster_object.convex_hull = in_detections.objects[i].convex_hull;

        clusters_objects.clusters.push_back(cluster_object);
    }
    _pub_clusters_message.publish(clusters_objects);
}
                                                ...
                                                const grid_map_msgs::GridMap::ConstPtr &in_gridmap)
{
  ...
  publishCloudClusters(in_roi_objects);
...
RosRoiObjectFilterApp::InitializeRosIo(ros::NodeHandle &in_private_handle)
{
  ...
  _pub_clusters_message = node_handle_.advertise<autoware_msgs::CloudClusterArray>("/cloud_clusters_filter", 1);
