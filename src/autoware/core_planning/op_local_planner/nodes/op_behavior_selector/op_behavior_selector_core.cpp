...
BehaviorGen::BehaviorGen()
{
  ...
  sub_GoalRemainingDistance = nh.subscribe("/goal_remaining_distance", 1, &BehaviorGen::callbackGetGoalRemainingDistance, this);  
...
void BehaviorGen::callbackGetGoalRemainingDistance(const std_msgs::Float32ConstPtr &msg)
{
...
void BehaviorGen::callbackGetGlobalPlannerPath(const autoware_msgs::LaneArrayConstPtr& msg)
{
    ...
    // m_BehaviorGenerator.m_pCurrentBehaviorState = m_BehaviorGenerator.m_pInitState;  // 在导航到下一个目标点的时候切换
    if (msg->lanes.size() > 0 && bMap)
    {

        bool bOldGlobalPath = m_GlobalPaths.size() == msg->lanes.size();

        m_baseWaypoints_.waypoints.clear();
        m_GlobalPaths.clear();

        m_baseWaypoints_ = msg->lanes[0];
...
void BehaviorGen::SendLocalPlanningTopics()
{
  ...
  pub_LocalBasePath.publish(m_baseWaypoints_);
...
void BehaviorGen::MainLoop()
{
  ...
  while (ros::ok())
  {
    ...
    if(bNewCurrentPos && m_GlobalPaths.size()>0)
    {
      ...
      // m_CurrentBehavior = m_BehaviorGenerator.DoOneStep(dt, m_CurrentPos, m_VehicleStatus, 1, m_CurrTrafficLight, m_TrajectoryBestCost, 0 );     // /roi_signal 通过位置确定红绿灯颜色
      m_CurrentBehavior = m_BehaviorGenerator.DoOneStep(dt, m_CurrentPos, m_VehicleStatus, 1, m_PrevTrafficLight, m_TrajectoryBestCost, 0 );       // /light_color 使用这个话题
