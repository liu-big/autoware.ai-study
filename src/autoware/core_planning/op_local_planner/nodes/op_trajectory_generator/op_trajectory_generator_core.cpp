...
TrajectoryGen::TrajectoryGen()
{
  ...
  sub_GoalRemainingDistance = nh.subscribe("/goal_remaining_distance", 1, &TrajectoryGen::callbackGetGoalRemainingDistance, this);
}
...
void TrajectoryGen::callbackGetGoalRemainingDistance(const std_msgs::Float32ConstPtr &msg)
{
...
void TrajectoryGen::MainLoop()
{
  ...
  while (ros::ok())
  {
    ...
    // m_PlanningParams.microPlanDistance = m_PlanningParams.minDistanceToAvoid + fabs(m_VehicleStatus.speed * 3.6 * 1.5) + 1;
    ...
    //   static int roll_number_ = m_PlanningParams.rollOutNumber;
    //   if(m_fRemainingDistance < 10)
    //         m_PlanningParams.rollOutNumber = 1;
    //   else
    //       m_PlanningParams.rollOutNumber = roll_number_;
      ...
      for(unsigned int i=0; i < m_RollOuts.size(); i++)   // 车道数
      {
        for(unsigned int j=0; j < m_RollOuts.at(i).size(); j++)   // 每个车道生成的局部车道数
        {
