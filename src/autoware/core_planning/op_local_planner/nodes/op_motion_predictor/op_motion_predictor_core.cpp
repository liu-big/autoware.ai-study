...
//    }
  ...
  if(bMap)
  {
    ...
    health_checker_ptr_->NODE_ACTIVATE();   // NODE_DEACTIVATE
    health_checker_ptr_->CHECK_RATE("topic_rate_op_motion_predictor_slow", 8, 5, 1,
                                    "topic motion_predictor publish rate slow.");
    m_objects_count = 0;
  ...
  for(unsigned int ic = 0; ic < m_Map.curbs.size(); ic++)
  {
    ...
    if(m_Map.curbs.at(ic).points.size() > 0)
    {
      ...
      if(curb_obstacles.size()>0)
      {
        ...
        if(distance_to_prev < m_DistanceBetweenCurbs)    // 过滤2个curb距离间的点
          continue;
      }

      double longitudinalDist = hypot(m_CurrentPos.pos.y -obj.center.pos.y, m_CurrentPos.pos.x-obj.center.pos.x);

      if(longitudinalDist > m_PlanningParams.horizonDistance)  // 超过了120米进行过滤
        continue;

      // if (longitudinalDist < (m_PlanningParams.horizontalSafetyDistancel + m_CarInfo.length +3.0)) // 过滤车身及安全距离再加上2米
      //     continue;
...
//    }
    ...
    if(health_checker_ptr_->getNodeStatus()){
      m_objects_count++;
      if(m_objects_count > 15){
        health_checker_ptr_->NODE_DEACTIVATE();
      }
    }
