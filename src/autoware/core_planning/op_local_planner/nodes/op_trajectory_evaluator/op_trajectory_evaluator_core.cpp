...
// void TrajectoryEval::emergency_pose(vector_map_msgs::PointArray points)
// {
//     double distance;
...
void TrajectoryEval::MainLoop()
{
  ...
  while (ros::ok())
  {
    ...
    // m_PlanningParams.minFollowingDistance = m_PlanningParams.minDi0stanceToAvoid + fabs(m_VehicleStatus.speed * 3.6 * 1.5) + 1;
    if(bNewCurrentPos && m_GlobalPaths.size()>0)
    {
      ...
      if(m_GlobalPathSections.size()>0)     // 为什么只取 m_GlobalPathSections.at(0) , 局部路径: m_GeneratedRollOuts, 全局路径: m_GlobalPathSections
      {
