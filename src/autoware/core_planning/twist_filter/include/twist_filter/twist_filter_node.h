...
private:
  ...
  int _use_gnss = 1;
  bool m_topic_status;
  double gnss_reinit_fitness;
  bool ndt_flag;

  int lane_id;
  double back_distance;
  bool is_blocked;
  bool b_obstacle_status;
  ...
  bool use_sensor_data_filter_;

  twist_filter::Ctrl ctrl_save;
  twist_filter::Twist mysmoothTwist(const twist_filter::Twist &twist);
  ...
  ros::Publisher brake_mode_pub_;

  // subscribers
  ros::Subscriber twist_sub_, ctrl_sub_, config_sub_,gpsfix_sub_,node_status_sub_,sub_ndtStat_sub_,sub_Trajectory_Cost,spd_ratio_sub;
  ...
  void callbackFromNdtStat(const autoware_msgs::NDTStatConstPtr &msg);
