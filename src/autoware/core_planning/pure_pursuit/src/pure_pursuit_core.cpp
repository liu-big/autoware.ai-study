...
namespace waypoint_follower
{
    ...
    PurePursuitNode::PurePursuitNode()
        : private_nh_("~"), pp_(), LOOP_RATE_(30), is_waypoint_set_(false), is_pose_set_(false), is_velocity_set_(false), current_linear_velocity_(0), command_linear_velocity_(0), direction_(LaneDirection::Forward), velocity_source_(-1), const_lookahead_distance_(4.0), const_velocity_(5.0), lookahead_distance_ratio_(2.0), minimum_lookahead_distance_(6.0), angular_multiple_(1.0)
    {
        initForROS();
        health_checker_ptr_ =
    ...
    void PurePursuitNode::initForROS()
    {
        ...
        private_nh_.param("angular_multiple", angular_multiple_, 1.0);
        ...
        sub1_ = nh_.subscribe("final_waypoints", 10,
                              &PurePursuitNode::callbackFromWayPoints, this);  //订阅最终waypoints
        sub2_ = nh_.subscribe("current_pose", 10,
                              &PurePursuitNode::callbackFromCurrentPose, this);  // 订阅当前位姿
        sub3_ = nh_.subscribe("config/waypoint_follower", 10,
                              &PurePursuitNode::callbackFromConfig, this);  // 订阅配置参数
        sub4_ = nh_.subscribe("current_velocity", 10,
                              &PurePursuitNode::callbackFromCurrentVelocity, this); // 订阅当前速度
    ...
    {
        ROS_INFO("can_get_curvature: %d, kappa: %f", can_get_curvature, kappa);
        geometry_msgs::TwistStamped ts;
        ts.header.stamp = ros::Time::now();
        ts.twist.linear.x = can_get_curvature ? computeCommandVelocity() : 0;
        ts.twist.angular.z = can_get_curvature ? kappa * ts.twist.linear.x*angular_multiple_ : 0;//angular_multiple_仿真为2
        pub1_.publish(ts);
    }
    ...
    void PurePursuitNode::publishControlCommandStamped(
        const bool &can_get_curvature, const double &kappa) const
    {
        ROS_INFO("publishes_for_steering_robot_: %d", publishes_for_steering_robot_);
        ...
        ccs.cmd.steering_angle =
            can_get_curvature ? convertCurvatureToSteeringAngle(wheel_base_, kappa) : 0; // 得到前轮转角
    ...
    double PurePursuitNode::computeLookaheadDistance() const
    {
        ...
        double tmp =  ld < minimum_lookahead_distance_ ? minimum_lookahead_distance_ : ld > maximum_lookahead_distance ? maximum_lookahead_distance
                                                                                                                : ld;
        tmp = (old_tmp + tmp) / 2.0;
        old_tmp = tmp;
        return tmp;
