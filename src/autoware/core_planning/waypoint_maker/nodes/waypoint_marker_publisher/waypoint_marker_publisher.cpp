...
ros::Publisher g_through_mark_pub;
...
void createLocalsafeMarker(const autoware_msgs::Lane& lane_waypoint)
{
  visualization_msgs::Marker lane_waypoint_marker;
  lane_waypoint_marker.header.frame_id = "map";
  lane_waypoint_marker.header.stamp = ros::Time::now();
  lane_waypoint_marker.ns = "local_safe_marker";
  lane_waypoint_marker.id = 0;
  lane_waypoint_marker.type = visualization_msgs::Marker::SPHERE_LIST;
  lane_waypoint_marker.action = visualization_msgs::Marker::ADD;

  lane_waypoint_marker.scale.x = safe_distance;
  lane_waypoint_marker.scale.y = safe_distance;
  lane_waypoint_marker.scale.z = safe_distance;
  ...
  for (unsigned int i = 0; i < lane_waypoint.waypoints.size(); i++)
  {
    ...
    //lane_waypoint_marker.color.g = lane_waypoint.waypoints[i].twist.twist.linear.x/10.0;
...
{
    static int devmode_old = msg->devmode;
    if(devmode_old != msg->devmode){
        mode_update_flag = true;
        devmode_old = msg->devmode;
    }
}

void getCurrentPoseCallback(const geometry_msgs::PoseStampedConstPtr &msg)
{
    static geometry_msgs::Pose old_pose = msg->pose;
    static geometry_msgs::PoseArray poses;

    geometry_msgs::Pose current_pose = msg->pose;
    double min_add_scan_shift = 0.5;

    static visualization_msgs::Marker through_waypoint_marker;
    if(through_waypoint_marker.points.size() == 0){
        through_waypoint_marker.header.frame_id = "map";
        through_waypoint_marker.header.stamp = ros::Time::now();
        through_waypoint_marker.ns = "through_path_marker";
        through_waypoint_marker.id = 0;
        through_waypoint_marker.type = visualization_msgs::Marker::LINE_STRIP;
        through_waypoint_marker.action = visualization_msgs::Marker::ADD;
        through_waypoint_marker.scale.x = 0.5;
        through_waypoint_marker.color.b = 1.0;
        through_waypoint_marker.color.a = 0.8;
        through_waypoint_marker.frame_locked = true;
    }

    double shift = sqrt(pow(current_pose.position.x - old_pose.position.x, 2.0) + pow(current_pose.position.y - old_pose.position.y, 2.0));
    if (shift >= min_add_scan_shift)
    {

        geometry_msgs::Point point = current_pose.position;
        through_waypoint_marker.points.push_back(point);

        g_through_waypoints_marker_array.markers.push_back(through_waypoint_marker);

        if(through_waypoint_marker.points.size() > 2000){     // 设置最大长度跟踪线段
            through_waypoint_marker.points.erase(through_waypoint_marker.points.begin());
        }
        if(mode_update_flag){                                 // 档位变化后重新清除所有点
            through_waypoint_marker.points.clear();
            mode_update_flag = false;
        }

        g_through_mark_pub.publish(g_through_waypoints_marker_array);
...
int main(int argc, char** argv)
{
  ...
  // 获取横向安全距离
  double car_width, horizontalSafetyDistance;
  nh.param("/op_common_params/width", car_width, 1.0);
  nh.param("/op_common_params/horizontalSafetyDistance", horizontalSafetyDistance, 0.5);

  safe_distance = car_width + horizontalSafetyDistance;
  ...
  ros::Subscriber current_pose_sub = nh.subscribe("/current_pose", 1, getCurrentPoseCallback);
  ...
  g_through_mark_pub = nh.advertise<visualization_msgs::MarkerArray>("through_waypoints_mark", 10, true);
