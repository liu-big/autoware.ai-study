...
void geo_pos_conv::set_err(double x, double y,double z,double yaw)
{
  err_x = x;
  err_y = y;
  err_z = z;
...
void geo_pos_conv::gnss_to_map()
{
   double yaw =  err_yaw;
   double m_z_ = m_z - err_z;

  double m_x_ = (m_x) * cos(yaw) + (m_y) * sin(yaw);
  double m_y_ = (m_y) * cos(yaw) - (m_x) * sin(yaw);
  double d = sqrt(m_x_*m_x_ +m_y_*m_y_ +m_z_*m_z_)/sqrt(m_x_*m_x_ +m_y_*m_y_ );
  d = 1;
   m_x = m_x_ * d;
   m_y = m_y_ * d;
   m_z = m_z_;
...
{
  float lon_deg, lon_min, lat_deg, lat_min;  // longitude and latitude of origin of each plane in Japan

  lon_deg = err_x; // 22.72880729;
  lon_min = 0;
  lat_deg = err_y; //114 .37654619;
  lat_min = 0;
...
//米勒投影   
    ...
    double x = lond/100.0 * M_PI / 180;                // 将经度从度数转换为弧度
    double y = latd/100.0 * M_PI / 180;                // 将纬度从度数转换为弧度
...
void geo_pos_conv::conv_llh2xyz(void)
{
  ...
  m_y = static_cast<double>(((PS - PSo) + (1.0 / 2.0) * PN * std::pow(std::cos(m_lat), 2.0) * Pt * std::pow(PDL, 2.0) +
  ...
  m_x = static_cast<double>((PN * std::cos(m_lat) * PDL -
