...
void DecisionMaker::InitBehaviorStates()
{
     ...
     const TrajectoryCost& bestTrajectory)
 {
   ...
   if(m_CarInfo.max_deceleration != 0)
     pValues->minStoppingDistance = 0;//-pow(car_state.speed, 2)/(m_CarInfo.max_deceleration);
  ...
  // 如果红绿灯超过m_params.giveUpDistance这个距离就放弃控制,后面的参数红绿灯前多少米开始识别
     if(distanceToClosestStopLine > m_params.giveUpDistance && distanceToClosestStopLine < -m_params.giveUpDistance)//(pValues->minStoppingDistance + 1.0))
   {
     if(m_pCurrentBehaviorState->m_pParams->enableTrafficLightBehavior)
     {
       ...
       {
         if(true /*detectedLights.at(i).id == trafficLightID*/)
 ...
 bool DecisionMaker::SelectSafeTrajectory()
 {
  ...
  double time_elapsed = (ros::Time::now() - last_callback_time).toSec();


  int currIndex = PlannerHNS::PlanningHelpers::GetClosestNextPointIndexFast(m_Path_src, state);
  int index_limit = 0;
  if(index_limit<=0)
    index_limit =  m_Path_src.size()/6.0;
  if(currIndex > index_limit              // 走到规划的一半刷新
      || preCalcPrams->bRePlan
      || preCalcPrams->bNewGlobalPath
      //|| time_elapsed > 2.0               // 时间大于5s更新,通过时间触发更新
      )
  {
    // 局部规划选中的行使线
    std::cout << "New Local Plan : " << currIndex << ", "<< preCalcPrams->bRePlan << ", " << preCalcPrams->bNewGlobalPath  << ", " <<  m_TotalOriginalPath.at(0).size() << ", PrevLocal: " << m_Path_src.size();
    m_Path_src = m_RollOuts.at(preCalcPrams->iCurrSafeTrajectory);
    std::cout << ", NewLocal: " << m_Path_src.size() << std::endl;
 ...
 PlannerHNS::BehaviorState DecisionMaker::GenerateBehaviorState(const PlannerHNS::VehicleState& vehicleState)
 {
  ...
  average_braking_distance = 0; //m_params.additionalBrakingDistance; // 固定该值

  if(average_braking_distance  < m_params.minIndicationDistance)
    average_braking_distance = m_params.minIndicationDistance;

  currentBehavior.indicator = PlanningHelpers::GetIndicatorsFromPath(m_Path_src, state, average_braking_distance );
 ...
 double DecisionMaker::UpdateVelocityDirectlyToTrajectory(const BehaviorState& beh, const VehicleState& CurrStatus, const double& dt)
 {
  ...
  PlanningHelpers::GetRelativeInfo(m_Path_src, state, info);
  double average_braking_distance = -pow(CurrStatus.speed, 2)/(m_CarInfo.max_deceleration) + m_params.additionalBrakingDistance;
  average_braking_distance = 0; //m_params.additionalBrakingDistance;    // 固定该值

  double max_velocity  = PlannerHNS::PlanningHelpers::GetVelocityAhead(m_TotalOriginalPath.at(m_iCurrentTotalPathId), total_info, total_info.iBack, average_braking_distance);  // 全局路径点的速度

  unsigned int point_index = 0;
  double critical_long_front_distance = m_CarInfo.length/2.0;

  double desiredVelocity = 0;
  double count_dis= max_velocity *2; //  20.0;
  double scale = 1;

  if(beh.state == TRAFFIC_LIGHT_STOP_STATE || beh.state == STOP_SIGN_STOP_STATE)
  {
    PlanningHelpers::GetFollowPointOnTrajectory(m_Path_src, info, beh.stopDistance - critical_long_front_distance, point_index);

    double e = -beh.stopDistance;
    double desiredVelocity = 0; //m_pidStopping.getPID(e);
...
//    std::cout << "Stopping : e=" << e << ", desiredPID=" << desiredVelocity << ", PID: " << m_pidStopping.ToString() << std::endl;
    ...
    for(unsigned int i =  0; i < m_Path.size(); i++)    // m_Path_src 为新选中的安全车道, 没更新就一直不变
  ...
  else if(beh.state == FOLLOW_STATE)
  {
    ...
    // beh.followDistance 与障碍物距离
      ...
      double desiredVelocity = (deceleration_critical * dt) + CurrStatus.speed;       // 使用最大加速度进行减速度
    #else


      int currIndex = PlannerHNS::PlanningHelpers::GetClosestNextPointIndexFast(m_Path_src, state) + beh.followDistance * (1 / m_params.pathDensity);  // + 的是当前位置到停车点的局部车道点数
      if(currIndex < 0 || currIndex >= m_Path_src.size()){
        return 0;
      }
      std::vector<WayPoint> tmp_path(m_Path_src.begin(), m_Path_src.begin() + currIndex);
      m_Path = tmp_path;

      // double desiredVelocity = m_params.maxSpeed;
      // if(beh.followDistance < m_params.maxDistanceToAvoid){
      //     desiredVelocity = 0.0;
      // }else if(beh.followDistance < m_params.minDistanceToAvoid){
      //     desiredVelocity = m_params.maxSpeed/2.0;
      // }
      // double desiredVelocity = (beh.followDistance > m_params.maxDistanceToAvoid) ? m_params.maxSpeed/2.0 : 0;
      ...
      for(unsigned int i = 0; i < m_Path.size(); i++)
      {
        desiredVelocity = max_velocity;
        // 如果剩余车道长度小于 n米进行减速
        if( ((m_Path.size() - i) * m_params.pathDensity) < count_dis ){    // m_Path * m_params.pathDensity 单位m

          scale = ((m_Path.size()- i) * m_params.pathDensity -1 )/ count_dis;
          desiredVelocity *= scale;                       // 减速比后的速度

          // if(desiredVelocity > CurrStatus.speed)
          //   desiredVelocity = CurrStatus.speed;
        }

      // if((desiredVelocity < 0.1 && desiredVelocity > -0.1) || beh.followDistance <= m_params.maxDistanceToAvoid) //use only effective velocities
      //   desiredVelocity = 0;

      // else if(desiredVelocity <= m_params.maxDeceleration && desiredVelocity > 0.3){
      //   desiredVelocity = m_params.maxDeceleration;
      // }

      //std::cout << "Acc: V: " << desiredVelocity << ", Accel: " << deceleration_critical<< std::endl;

      // for(unsigned int i = 0; i < m_Path_src.size(); i++)
      ...
      if(i == m_Path.size()-1){
        ...
        if( desiredVelocity < m_params.minSpeed)
          desiredVelocity = m_params.minSpeed;
        else if(desiredVelocity > m_params.maxSpeed)
          desiredVelocity = m_params.maxSpeed;
        else if(desiredVelocity < 0)
          desiredVelocity = 0;
      }
      m_Path.at(i).v = desiredVelocity;                 // 华仔,速度
    }

    if(m_Path.size() > 0)
      desiredVelocity = m_Path.at(0).v;
    return desiredVelocity;
  ...
  else if(beh.state == FORWARD_STATE || beh.state == OBSTACLE_AVOIDANCE_STATE )
  {
    ...
    //std::cout << "Target Velocity: " << desiredVelocity << ", Change Slowdown: " << bSlowBecauseChange  << std::endl;
      ...
      if(m_pCurrentBehaviorState->GetCalcParams()->iCurrSafeTrajectory != m_pCurrentBehaviorState->GetCalcParams()->iCentralTrajectory)
      {
        target_velocity*=0.5;              // 不走中心轨迹速度减半
        bSlowBecauseChange = true;
      }
    ...
    const bool& bEmergencyStop)
{
  ...
  beh = GenerateBehaviorState(vehicleState);    // pCParams->bRePlan = true; 可局部变道标志

  beh.bNewPlan = SelectSafeTrajectory();        // 局部变道更新
