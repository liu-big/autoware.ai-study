...
//      cout << " Alert Alert !!!! fast: " << info.iFront << ", slow: " << old_index  << endl;
  ...
  Mat3 translationMat(-p.pos.x, -p.pos.y); // 把p改为p0,就在全局点开始规划
...
int PlanningHelpers::GetClosestNextPointIndexFast(const vector<WayPoint>& trajectory, const WayPoint& p,const int& prevIndex )
{
  int size = (int)trajectory.size();
    ...
    int iEnd = size;
    double resolution = hypot(trajectory[1].pos.y -trajectory[0].pos.y , trajectory[1].pos.x -trajectory[0].pos.x);    // 每个点的分辨率
    ...
    std::vector<WayPoint>& sampledPoints)
{
  ...
  //Get Closest Index
  RelativeInfo info;
  GetRelativeInfo(originalCenter, carPos, info);    // originalCenter 为全局路径
...
//  }
  ...
  for(int i=0; i< rollOutNumber+1; i++)
  {
    double end_roll_in_distance = rollOutDensity*(i - centralTrajectoryIndex);
      // double end_roll_in_distance = rollOutDensity * (i - centralTrajectoryIndex) * pow((fabs(i - centralTrajectoryIndex) / centralTrajectoryIndex), 0.65);  // 指数为0,变换越小，为1时变换最大。不改变总体的范围大小
  ...
  for(unsigned int i=0; i< rollOutNumber+1 ; i++)   //前段 roll in路径
...
void PlanningHelpers::ExtractPlanAlernatives(const std::vector<WayPoint>& singlePath, std::vector<std::vector<WayPoint> >& allPaths)
{
    ...
    if(singlePath.at(i).bDir != FORWARD_DIR && singlePath.at(i).pLane && singlePath.at(i).pFronts.size() > 0)
    {
      ...
      if(pLaneCell)
      {
        ...
        if(straight_path.size() > 2)
        {
          ...
          allPaths.push_back(straight_path);                             // first lane
        }
      }
    }
  ...
  allPaths.push_back(path); // second lane
}
