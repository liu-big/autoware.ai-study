    ...
    std::vector<WayPoint>& sampledPoints_debug)
{
  ...
  for(unsigned int i = 0; i < referencePaths.size(); i++)
  {
    ...
    rollOutsPaths.push_back(local_rollOutPaths);    // roll out 段
  }
}
...
{
  PlannerHNS::WayPoint* pStart = PlannerHNS::MappingHelpers::GetClosestWaypointFromMap(start, map);         // ��ȡ�����ǰλ��lane
  PlannerHNS::WayPoint* pGoal = PlannerHNS::MappingHelpers::GetClosestWaypointFromMap(goalPos, map);        // ��ȡĿ���λ��lane
  ...
  if(all_cell_to_delete)
    ...
    pLaneCell =  PlanningHelpers::BuildPlanningSearchTreeV2(pStart, *pGoal, globalPath, maxPlanningDistance,bEnableLaneChange, local_cell_to_delete);  // 这里是否变道控制
  ...
  if(bPlan == 'A')
  {
    PlanningHelpers::ExtractPlanAlernatives(path, paths);  // gen
  }
  else if (bPlan == 'B')
  {
    paths.push_back(path);
