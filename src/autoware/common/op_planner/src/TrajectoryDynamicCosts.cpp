    ...
    const std::vector<PlannerHNS::DetectedObject>& obj_list, const int& iCurrentIndex)
{
  ...
  double critical_lateral_distance   =  carInfo.width/2.0 + params.horizontalSafetyDistancel;                           // 横向距离的一半
  ...
  // double critical_long_back_distance   =  carInfo.length/2.0 + 0.2 - carInfo.wheel_base/2.0;   // 设置后面识别障碍物距离小
  ...
  InitializeCosts(rollOuts, params); // m_TrajectoryCosts得到基本的参

  InitializeSafetyPolygon(currState, carInfo, vehicleState, critical_lateral_distance, critical_long_front_distance, critical_long_back_distance);   // 得到safe距离的6点坐标

  CalculateTransitionCosts(m_TrajectoryCosts, currIndex, params); // 得到过渡成本
  //计算横向和纵向成本动态
  ...
  NormalizeCosts(m_TrajectoryCosts);    // 得到每条lane的归一化的代价
  ...
  for(unsigned int ic = 0; ic < m_TrajectoryCosts.size(); ic++)
  {
    ...
    if(!m_TrajectoryCosts.at(ic).bBlocked && m_TrajectoryCosts.at(ic).cost < smallestCost)        // 选择 lane,及其代价值
    {
      smallestCost = m_TrajectoryCosts.at(ic).cost;
      smallestIndex = ic;
    }

    if(m_TrajectoryCosts.at(ic).closest_obj_distance < smallestDistance)                         // 得到离障碍物最近的距离
    {
  ...
  if(smallestIndex == -1)   // 当全部不能通行的时候才执行这里
  {

    smallestIndex = params.rollOutNumber / 2;
    smallestDistance = m_TrajectoryCosts.at(smallestIndex).closest_obj_distance;
    ...
    const std::vector<PlannerHNS::DetectedObject>& obj_list, const int& iCurrentIndex)
{
  ...
  if(rollOuts.size()>0)
  {
    ...
    for(unsigned int it=0; it< rollOuts.size(); it++)
    {
      ...
      // tc.distance_from_center *= pow((fabs(tc.relative_index) / centralIndex), 0.65); // 指数为0,变换越小，为1时变换最大。不改变总体的范围大小(解决过小地方)
  ...
  static int sPrevLaneIndex = currIndex;
  CalculateTransitionCosts(m_TrajectoryCosts, currIndex, params); 
  // CalculateTransitionCosts(m_TrajectoryCosts, sPrevLaneIndex, params); // 修改为真实的过渡成本
  ...
  for(unsigned int io=0; io<obj_list.size(); io++)
  {
    for(unsigned int icon=0; icon < obj_list.at(io).contour.size(); icon++)
    {
      ...
      p.id = io;
      p.cost = sqrt(obj_list.at(io).w*obj_list.at(io).w + obj_list.at(io).l*obj_list.at(io).l);  // 障碍物欧式距离代价
      m_AllContourPoints.push_back(p);
    }
  }

  CalculateLateralAndLongitudinalCostsStatic(m_TrajectoryCosts, rollOuts, totalPaths, currState, m_AllContourPoints, params, carInfo, vehicleState); // 得到safe距离的6点坐标,和计算横向和纵向成本动态
  ...
  if(smallestIndex == -1)
  {
    ...
    smallestIndex = floor(params.rollOutNumber / 2);
    smallestDistance = m_TrajectoryCosts.at(smallestIndex).closest_obj_distance;
  ...
  {
    bestTrajectory = m_TrajectoryCosts.at(smallestIndex);
    bestTrajectory.closest_obj_distance = smallestDistance;         // 得到障碍物最近距离
  }

  m_PrevIndex = currIndex;
  sPrevLaneIndex = bestTrajectory.index;
  return bestTrajectory;
}
...
{
  double horizontalSafetyDistancel_ = 1.0;
//   if (vehicleState.speed == 0.0)
//   {
//       horizontalSafetyDistancel_ = 0.5;
//   }else if(vehicleState.speed < 0.5){
//       horizontalSafetyDistancel_ = 0.75;
//   }
  double critical_lateral_distance = carInfo.width / 2.0 + params.horizontalSafetyDistancel * horizontalSafetyDistancel_;
  double critical_long_front_distance =  carInfo.wheel_base/2.0 + carInfo.length/2.0 + params.verticalSafetyDistance;
  // double critical_long_back_distance =  carInfo.length/2.0 + params.verticalSafetyDistance - carInfo.wheel_base/2.0;    // 设置后面识别障碍物距离小
  double critical_long_back_distance   =  carInfo.length/2.0 + 0.30 - carInfo.wheel_base/2.0;  
...
//          if(close_in_percentage <= 0 || close_in_percentage > 1) close_in_percentage = 1;
        ...
        bool safe_on_ = false;
        if(m_SafetyBorder.PointInsidePolygon(m_SafetyBorder, contourPoints.at(icon).pos) == true){   // 障碍物是否与安全距离相交
        ...
        if((lateralDist - carInfo.width/3.0) <= critical_lateral_distance
            && longitudinalDist >= -carInfo.length/1.0     // 修改1.5
            && longitudinalDist < params.minFollowingDistance)   // 跟车距离
          trajectoryCosts.at(iCostIndex).bBlocked = true;                                    


        if(lateralDist != 0)
          trajectoryCosts.at(iCostIndex).lateral_cost += 1.0/lateralDist;    // 反向,值越大影响越小。
        ...
        if(longitudinalDist >= -critical_long_front_distance && longitudinalDist < trajectoryCosts.at(iCostIndex).closest_obj_distance
                    && trajectoryCosts.at(iCostIndex).bBlocked && (lateralDist ) <= critical_lateral_distance )  //如果该路径阻塞才得到距离,为了跟车功能
        {
        ...
        if(safe_on_ == true){
            trajectoryCosts.at(iCostIndex).closest_obj_distance = 0;
            trajectoryCosts.at(iCostIndex).closest_obj_velocity = 0;
...
//  cout << "------ Normalizing Step " << endl;
  for(unsigned int ic = 0; ic< trajectoryCosts.size(); ic++)
  {
    ...
    trajectoryCosts.at(ic).cost = (m_WeightPriority*trajectoryCosts.at(ic).priority_cost + m_WeightTransition*trajectoryCosts.at(ic).transition_cost + 0*trajectoryCosts.at(ic).lateral_cost + m_WeightLong*trajectoryCosts.at(ic).longitudinal_cost)/4.0;
    // trajectoryCosts.at(ic).cost = (m_WeightPriority*trajectoryCosts.at(ic).priority_cost + 0*trajectoryCosts.at(ic).transition_cost + 0*trajectoryCosts.at(ic).lateral_cost + 0*trajectoryCosts.at(ic).longitudinal_cost)/4.0;
...
void TrajectoryDynamicCosts::InitializeSafetyPolygon(const WayPoint& currState, const CAR_BASIC_INFO& carInfo, const VehicleState& vehicleState, const double& c_lateral_d, const double& c_long_front_d, const double& c_long_back_d)
{
  ...
  double ratio_to_angle = corner_slide_distance/carInfo.max_steer_angle;
  double slide_distance = vehicleState.steer * ratio_to_angle;              // 融合了角度的值,还可以融合速度值
    ...
    const VehicleState& vehicleState, const double& c_lateral_d, const double& c_long_front_d, const double& c_long_back_d )
{
  ...
  {
    if(obj_list.at(i).label.compare("curb") == 0)   // 过滤掉近距离的curb点
    {
    ...
    if(obj_list.at(i).bVelocity && obj_list.at(i).predTrajectories.size() > 0) // dynamic  , 有障碍物在车道内就执行这里
    {

      for(unsigned int ir=0; ir < rollOuts.size(); ir++)   //rollOuts :waypoint lane表示路线
      {
        WayPoint collisionPoint;
        TrajectoryCost trajectoryCosts;
        CalculateIntersectionVelocities(rollOuts.at(ir), obj_list.at(i), currState, carInfo, c_lateral_d, collisionPoint, trajectoryCosts); // 计算交点速度
        if(trajectoryCosts.bBlocked)
        {
          ...
          m_CollisionPoints.push_back(collisionPoint);  // 碰撞点
        }
      }
    }
    else
